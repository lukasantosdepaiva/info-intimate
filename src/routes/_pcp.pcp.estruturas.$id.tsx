...
function EstruturaEditor() {
  ...
  const fetchEstrutura = useCallback(async () => {
    ...
    try {
      ...
      setEstrutura(e);
      setReferenciaId(e.produto_pai_id);
      ...
      const { data: its } = await supabase
        .from("estruturas_produto_itens")
        .select("*")
        .eq("estrutura_id", id)
        .order("sequencia");
      setItens(
        ((its as EstruturaProdutoItem[]) ?? []).map((x) => ({
          id: x.id,
          componente_id: x.produto_componente_id,
          quantidade: Number(x.quantidade),
          indice_perda: Number(x.indice_perda ?? 0),
  ...
  const salvar = async () => {
    ...
    if (itens.length === 0) {
      toast.error("Adicione ao menos um componente.");
      return;
    }
    if (itens.some((it) => !it.componente_id || it.quantidade <= 0)) {
      toast.error("Preencha componente e quantidade em todos os itens.");
      return;
    }
    setSaving(true);
    try {
      ...
      if (isNew) {
        ...
        const { data: createdId, error: createError } = await (supabase as any).rpc(
          "salvar_estrutura_produto",
          {
            p_produto_pai_id: referenciaId,
            p_revisao: revisao,
            p_status: status,
            p_data_inicio_vigencia: inicioVig || null,
            p_observacao: observacao || null,
            p_itens: itens.map((it) => ({
              produto_componente_id: it.componente_id,
              quantidade: it.quantidade,
              indice_perda: it.indice_perda,
              sequencia: it.sequencia,
            })),
          },
        );
        if (createError) throw createError;
        const estId = createdId as string;
        if (!estId) throw new Error("A função não retornou a estrutura criada.");
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { error: eUpd } = await (supabase as any)
          .from("estruturas_produto")
          .update({
  ...
  const criarNovaRevisao = async () => {
    ...
    try {
      const supabase = getSupabase();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: novaId, error: revisionError } = await (supabase as any).rpc(
        "criar_revisao_estrutura",
        { p_estrutura_id: estrutura.id },
      );
      if (revisionError) throw revisionError;
      const novoId = novaId as string;
      if (!novoId) throw new Error("A função não retornou a nova revisão.");
  ...
  return (
    <main className="space-y-4">
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="number"
              value={revisao}
              onChange={(e) => setRevisao(Number(e.target.value))}
              disabled={!isNew}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              type="date"
              value={inicioVig}
              onChange={(e) => setInicioVig(e.target.value)}
              disabled={!isNew}
            />
          </div>
          <div className="space-y-1.5">
      ...
      <Card>
        ...
        <CardContent>
          {itens.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">Nenhum componente.</p>
          )}
          {itens.length > 0 && (
                    ...
                    <tr key={idx} className="border-b">
                      <td className="p-2">
                        <Input
                          type="number"
                          value={it.sequencia}
                          onChange={(e) => updateItem(idx, { sequencia: Number(e.target.value) })}
                          className="h-7 text-xs"
                          disabled={vigente || !canWrite}
                        />
                      </td>
                      <td className="p-2">
                        <Select
                          value={it.componente_id}
                          onValueChange={(v) => updateItem(idx, { componente_id: v })}
                          disabled={vigente || !canWrite}
                        >
                      ...
                      <td className="p-2">
                        <Input
                          type="number"
                          step="0.01"
                          value={it.indice_perda}
                          onChange={(e) =>
                            updateItem(idx, { indice_perda: Number(e.target.value) })
                          }
                          className="h-7 text-xs text-right"
                          disabled={vigente || !canWrite}
                        />
      ...
      {vigente && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-700 dark:text-amber-400">
          ℹ️ Esta é a revisão vigente. Para alterar itens, use <strong>Nova revisão</strong> — a
          atual será inativada automaticamente.
        </div>
      )}
    </main>
  );

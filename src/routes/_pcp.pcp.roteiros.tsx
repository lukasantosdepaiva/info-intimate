...
type RoteiroDbRow = RoteiroProducao & {
...
function RoteirosPage() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: e } = await supabase
        ...
        .select("*, referencias!produto_id(codigo_referencia)")
        .order("created_at", { ascending: false });
      if (e) throw new Error(e.message);
      setRows(
        ((data ?? []) as RoteiroDbRow[]).map((r) => ({
          ...r,
          codigo_referencia: r.referencias?.codigo_referencia ?? "—",
        })),
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : "Erro ao carregar roteiros.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        (r.codigo_referencia ?? "").toLowerCase().includes(q) || r.nome.toLowerCase().includes(q),
    );
  }, [rows, busca]);
        ...
        <div className="flex gap-2">
          ...
          {canWrite && (
            <Button size="sm" className="gap-1" asChild>
              <Link to="/pcp/roteiros/$id" params={{ id: "novo" }}>
